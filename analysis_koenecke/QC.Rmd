...
```{r loading data}
experimental_metadata = read.delim("analysis_koenecke/metadata.txt", sep=",", header=TRUE, stringsAsFactors=FALSE)
...
for( i in experimental_metadata$sample_id){
  data[,i] = read.table(paste("data/koenecke_rnaseq/", i, ".genes.results",sep=""), sep="\t", header=TRUE)$expected_count
}
row.names(data) = read.table(paste("data/koenecke_rnaseq/", i, ".genes.results",sep=""), sep="\t", header=TRUE)$gene_id

#Create a factor for the germ layer column - by making it a factor you give it an order
experimental_metadata$germ_layer = factor(experimental_metadata$germ_layer, levels=c("Ectoderm", "Mesoderm", "Neuroectoderm"))
...
dds = DESeqDataSetFromMatrix(data_mat, experimental_metadata, ~ germ_layer)
...
set.seed(1)
(pca_data <- plotPCA(rld, intgroup = c("germ_layer", "replicate"), returnData=TRUE))
percentVar <- round(100 * attr(pca_data, "percentVar"), digits=2)
ggplot(pca_data, aes(PC1, PC2, color=germ_layer, shape=replicate)) + geom_point(size=3) +
...
for( i in experimental_metadata$sample_id){
  effective_lengths[,i] = read.table(paste("data/koenecke_rnaseq/", i, ".genes.results",sep=""), sep="\t", header=TRUE)$effective_length
}
row.names(effective_lengths) = read.table(paste("data/koenecke_rnaseq/", i, ".genes.results",sep=""), sep="\t", header=TRUE)$gene_id
...
if(!dir.exists("analysis_koenecke/output")){
  dir.create("analysis_koenecke/output")
}

if(!dir.exists("analysis_koenecke/output/QC")){
  dir.create("analysis_koenecke/output/QC")
}

saveRDS(dds, "analysis_koenecke/output/QC/dds_quality_control.rds")
saveRDS(nctpm, "analysis_koenecke/output/QC/tpm.rds")
saveRDS(ensembl.genes, "analysis_koenecke/output/QC/ensembl_genes.rds")
saveRDS(dm.gtf.db, "analysis_koenecke/output/QC/dm_gtf_db.rds")
saveRDS(experimental_metadata, "analysis_koenecke/output/QC/experimental_metadata.rds")
```
